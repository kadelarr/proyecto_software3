angular.module('Usuario').service('UsuarioService', function($http) {
	...
	this.login = function(data, callback, callerror) {
		$http({
			method : 'POST',
			url : 'http://localhost:8082/proagro/rest/usuario/login',
			data : data,
			headers : {
	...
	this.logout = function(data, callback, callerror) {
		$http({
			method : 'POST',
			url : 'http://localhost:8082/proagro/rest/usuario/logout',
			data : data,
			headers : {
				'Content-Type' : 'application/x-www-form-urlencoded'
			}
	...
	this.listarUsuarios = function(callback, callerror) {
		$http({
			method : 'POST',
			url : 'http://localhost:8082/proagro/rest/usuario/listarUsuarios'
		}).success(function(response) {
			callback(response);
		}).error(function() {
			callerror();
